 ...
 */
class AnamiPagesFunctionTemplatePlugin implements IFunctionTemplatePlugin {
    const SHOW_LINKS = 9;
    ...
    protected function makeLink($link, $pageNo, $activePage, $pages) {
        ...
        if ($activePage != $pageNo) {
            ...
            // <span class="invisible">'.WCF::getLanguage()->getDynamicVariable('wcf.page.pagePosition', array('pageNo' => $pageNo, 'pages' => $pages)).'</span>
            return '<li class="active"><span>'.StringUtil::formatInteger($pageNo).'</span></li>';
        }
    }
    ...
    protected function makePreviousLink($link, $pageNo) {
        if ($pageNo > 1) {
            return '<li class="previous"><a href="'.$this->insertPageNumber($link, $pageNo - 1).'" rel="prev" role="button" title="'.WCF::getLanguage()->getDynamicVariable('wcf.global.page.previous').'" class="jsTooltip"><i class="fa fa-chevron-left"></i></a></li>';
        }
    }
    ...
    protected function makeNextLink($link, $pageNo, $pages) {
        if ($pageNo && $pageNo < $pages) {
            return '<li class="next"><a href="'.$this->insertPageNumber($link, $pageNo + 1).'" rel="next" role="button" title="'.WCF::getLanguage()->getDynamicVariable('wcf.global.page.next').'" class="jsTooltip"><i class="fa fa-chevron-right"></i></a></li>';
        }
    }
        ...
        if ($tagArgs['pages'] > 1) {
            ...
            if ($left > 1) {
                if ($left - 1 < 2) {
                    ...
                    $html .= '<li class="jumpTo ellipsis" role="separator"><span>'.StringUtil::HELLIP.'</span></li>';
                }
            }
